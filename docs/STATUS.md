...
**Carried over from the Personal-Trainer project (Flask).** Its 216-exercise library, mapped into this platform's taxonomy rather than imported raw (movement pattern, muscles, equipment class, difficulty) — the platform library is now 235. "Adjust today's workout" (sore / injured / equipment busy) resolved through the substitution graph and applied **session-scoped**, never rewriting the program as the original did. Structured health intake (experience, medical history, medications, surgical history, physician clearance) in its own encrypted table. External video URL on exercises as a stand-in until a gym films its own. See [DECISIONS.md](DECISIONS.md) D-018.
